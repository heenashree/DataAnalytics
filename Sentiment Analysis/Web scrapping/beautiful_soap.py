...
username = 'heenashree2015@gmail.com'
password = 'Epam123$$'
...
def sign_in():
    logger.info(f'Signing in to {sys.argv.username}')
...
def main():
    ...
    logger.info(f'Scraping up to {sys.argv.limit} reviews.')
    ...
    while more_pages() and\
            len(res) < sys.argv.limit and\
            not date_limit_reached[0]:
        go_to_next_page()
        reviews_df = extract_from_page()
        res = res.append(reviews_df)

    logger.info(f'Writing {len(res)} reviews to file {sys.argv.file}')
    res.to_csv(sys.argv.file, index=False, encoding='utf-8')
